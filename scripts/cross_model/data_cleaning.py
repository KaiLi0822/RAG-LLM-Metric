...
deepseek = load_dataset("RAGEVALUATION-HJKMY/DeepSeek7b_ragbench_delucionqa_400row_mistake_added", split="train").to_pandas()
llama = load_dataset("RAGEVALUATION-HJKMY/Llama8b_ragbench_delucionqa_400row_mistake_added", split="train").to_pandas()
mistral = load_dataset("RAGEVALUATION-HJKMY/Mistral8b_ragbench_delucionqa_400row_mistake_added", split="train").to_pandas()
qwen = load_dataset("RAGEVALUATION-HJKMY/Qwen7b_ragbench_delucionqa_400row_mistake_added", split="train").to_pandas()

# Extract metrics from DeepSeek dataset
metric_pattern = re.compile(r'^(Correct|Incorrect|ground_truth)_(.+)$')
metric_names = sorted({match.group(2) for col in deepseek.columns if (match := metric_pattern.match(col))})
