...
# Get metric names from the combined dataframe (supporting both with and without _score suffix)
metric_columns = [col for col in combined_df.columns if col.startswith("ground_truth_")]
metric_names = []
for col in metric_columns:
    raw = col.replace("ground_truth_", "")
    if raw.endswith("_score"):
        raw = raw[:-6]  # remove '_score'
    metric_names.append(raw)
...
for dataset in datasets:
    ...
    for metric in metric_names:
        try:
            gt_col = [col for col in filtered_df.columns if col.startswith(f"ground_truth_{metric}")][0]
            re_col = [col for col in filtered_df.columns if col.startswith(f"Correct_{metric}")][0]
            in_col = [col for col in filtered_df.columns if col.startswith(f"Incorrect_{metric}")][0]

            gt_scores = filtered_df[gt_col].dropna()
            re_scores = filtered_df[re_col].dropna()
            in_scores = filtered_df[in_col].dropna()
            ...
            pooled_sd = np.sqrt((gt_var * (len(gt_scores) - 1) + in_var * (len(in_scores) - 1)) / (len(gt_scores) + len(in_scores) - 2))
            ...
            print(f"Metric {metric} error for {dataset}. Error: {e}")
...
for metric in metric_names:
    try:
        gt_col = [col for col in combined_df.columns if col.startswith(f"ground_truth_{metric}")][0]
        re_col = [col for col in combined_df.columns if col.startswith(f"Correct_{metric}")][0]
        in_col = [col for col in combined_df.columns if col.startswith(f"Incorrect_{metric}")][0]

        gt_scores = combined_df[gt_col].dropna()
        re_scores = combined_df[re_col].dropna()
        in_scores = combined_df[in_col].dropna()
        ...
        print(f"Metric {metric} error in combined dataset. Error: {e}")
