...
class HFClientVLLM(LLMClient):
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        outputs = self.llm.generate(
            formatted_prompt, sampling_params=self.sampling_params, **kwargs
        )
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        if hasattr(self.tokenizer, "chat_template") and self.tokenizer.chat_template:
            ...
            formatted_prompt = self.tokenizer.decode(
                input_tensor[0], skip_special_tokens=True
            )
        else:
...
if __name__ == "__main__":
    ...
    http_llm = HTTPLLM()
    logger.info(
        "HTTP response:", http_llm.generate("Explain quantum computing in 3 sentences")
    )
